  ...
  try {
    ...
    if (!session?.mattJwtToken) {
      return NextResponse.json(
        { error: "Unauthorized - No Matt JWT token available" },
        { status: 401 }
      );
    }
    ...
    const response = await mattAPI.fetchActivities(
      session.mattJwtToken,
      filter
    );

    // Filter only commits
    const filteredCommits = response.activities.filter(
      (activity) => activity.type === "commit"
    ) as SimplifiedCommitDto[];

    console.log(
      `Filtered commits from last 30 days: ${filteredCommits.length} total commits`
    );
    ...
    for (const user of users) {
      ...
      commitsByUser[user.login] = {
        numberOfCommits: userAllCommits.length,
        commits: userAllCommits,
      };
      console.log(
        "Number of commits for user",
        user.login,
        userAllCommits.length
      );
    }
    ...
    if (
      ...
      (error.message === "JWT token has expired" ||
        error.message === "No JWT token provided")
    ) {
      return NextResponse.json(
